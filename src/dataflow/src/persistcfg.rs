...
impl PersistConfig {
    ...
    pub fn init(&self) -> Result<Option<PersisterWithConfig>, String> {
        if self.user_table_enabled {
            let buffer = FileBuffer::new(&self.buffer_path, self.lock_info.as_bytes())
                .map_err(|err| err.to_string())?;
            let blob = FileBlob::new(&self.blob_path, self.lock_info.as_bytes())
                .map_err(|err| err.to_string())?;
            let persister = Persister::new(buffer, blob).map_err(|err| err.to_string())?;
...
// TODO: This is a temporary placeholder until Row gets proper encode/decode
// methods. Also the String will be Vec<u8> once that's refactored in persist.
...
// TODO: This is a temporary placeholder until Row gets proper encode/decode
// methods. Also the String will be Vec<u8> once that's refactored in persist.
pub fn string_to_row(buf: &str) -> Result<Row, String> {
    let bytes = base64::decode(buf).map_err(|err| err.to_string())?;
    Ok(unsafe { Row::from_bytes_unchecked(bytes) })
}
